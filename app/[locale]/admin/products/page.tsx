...
export default function AdminProductsPage() {
  ...
  const [canvaDropdown, setCanvaDropdown] = useState<string | null>(null);

  // Close action menu and canva dropdown when clicking outside
  useEffect(() => {
    const handleClickOutside = () => {
      setActionMenu(null);
      setCanvaDropdown(null);
    };
    if (actionMenu || canvaDropdown) {
      document.addEventListener('click', handleClickOutside);
      return () => document.removeEventListener('click', handleClickOutside);
    }
  }, [actionMenu, canvaDropdown]);
              ...
              {loading ? (
                ...
                <tr key={product.id} className={`hover:bg-gray-50 transition-colors ${selectedProducts.has(product.id) ? 'bg-red-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    ) : (product.canvaTemplateLinks && product.canvaTemplateLinks.length > 0) || product.canvaTemplateLink ? (
                      (() => {
                        const links = product.canvaTemplateLinks || (product.canvaTemplateLink ? [{ name: 'Open Template', url: product.canvaTemplateLink }] : []);
                        if (links.length === 1) {
                          return (
                            <a
                              href={links[0].url}
                              ...
                              onClick={(e) => e.stopPropagation()}
                              className="inline-flex items-center gap-1 px-2.5 py-1 bg-gradient-to-r from-[#00C4CC] to-[#7B2FF7] text-white rounded-lg text-xs font-medium hover:opacity-90 transition-opacity cursor-pointer"
                            >
                        ...
                        return (
                          <div className="relative">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                setCanvaDropdown(canvaDropdown === product.id ? null : product.id);
                              }}
                              className="inline-flex items-center gap-1 px-2.5 py-1 bg-gradient-to-r from-[#00C4CC] to-[#7B2FF7] text-white rounded-lg text-xs font-medium hover:opacity-90 transition-opacity cursor-pointer"
                            >
                              <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
